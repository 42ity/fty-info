...
{
    fty_info_rc0_runonce_t* self = reinterpret_cast<fty_info_rc0_runonce_t*>(zmalloc(sizeof(*self)));
    if (self) {
        ...
        if (!(self->name && self->client && self->resolver && self->info)) {
            log_error("fty_info_rc0_runonce_new initialization failed");
...
{
    if (!(msgIn && fty_proto_is(msgIn))) {
        return 0;
    }

    fty_proto_t* message = NULL;
    {
        zmsg_t* dup = zmsg_dup(msgIn);
        message = dup ? fty_proto_decode(&dup) : NULL;
        zmsg_destroy(&dup);
    }
    ...
    if (!iname || !streq(iname, DEFAULT_RC_INAME)) {
    ...
    data = fty_proto_ext_string(message, "uuid", NULL);
    if (NULL == data) {
        ...
        if (getifaddrs(&interfaces) != -1) {
            ...
            for (struct ifaddrs *iface = interfaces; iface != NULL; iface = iface->ifa_next) {
                ...
                if ((iface->ifa_addr->sa_family == AF_INET)
                    && (getnameinfo(iface->ifa_addr, sizeof(struct sockaddr_in), host, NI_MAXHOST, NULL, 0, NI_NUMERICHOST) == 0)
                ) {
                    switch (counter) {
                        case 0:
    ...
    if (changeRO) {
        ...
        int r = mlm_client_sendto(self->client, AGENT_FTY_ASSET, "ASSET_MANIPULATION", NULL, timeout_ms, &msgDup);
        zmsg_destroy(&msgDup);
        if (r != 0) {
...
static int handle_pipe(fty_info_rc0_runonce_t* self, zmsg_t* message)
{
    ...
    if (!command) {
        ...
        if (stream && pattern) {
            ...
            log_error("%s: Undefined stream (%s) or pattern (%s)", self->name, stream, pattern);
        }
        zstr_free(&pattern);
        zstr_free(&stream);
    }
    else {
        log_error("%s: Unexpected command (%s)", self->name, command);
    }
...
void fty_info_rc0_runonce(zsock_t* pipe, void* args)
{
    ...
    if (!self) {
        log_error("%s: fty_info_rc0_runonce_new failed", name);
        return;
    }
    ...
    while (!zsys_interrupted) {
        ...
        if (which == NULL) {
            ...
            int r = handle_pipe(self, msg);
            zmsg_destroy(&msg);
            if (r != 0) {
                log_debug("%s: Broken pipe message", name);
                break;
            }
        }
            ...
            if (streq(command, "STREAM DELIVER")) {
                int r = handle_stream(self, msg);
                if (r < 0) {
                    log_debug("%s: Broken stream message", name);
                    term = true;
                }
                else if (r == 1) {
                    log_debug("%s: RC-0 updated, finishing", name);
                    term = true;
                }
            }
